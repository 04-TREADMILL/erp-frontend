<template>
  <div class="card">
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        <el-row>
          <el-col :span="3" v-if="check_type1(item)">
            <span><strong>供应商id: </strong>{{item.supplier}}</span>
          </el-col>
          <el-col :span="8" v-if="check_type2(item)">
        ...
        <div v-if="showAll[index] && check_type1(item)" style="margin-top: 15px">
        ...
        <div v-if="showAll[index] && check_type2(item)" style="margin-top: 15px">
          <div style="margin-bottom: 15px"><strong>详细商品清单:</strong></div>
          <el-table
...
export default {
  ...
  methods: {
    ...
    check_type1(item){
      if(item.supplier) return true
      return false
    },  
    check_type2(item){
      if(item.purchaseSheetId) return true
      return false
    },
    approval(id) {
      let config = {
